...
- All inputs have already been provided through the script.
...
unpack them, please run chmod +x on this file and run it. This will then
make all the other bash scripts executable so you only have to run the chmod command once.
...
please run client_trialTest.sh.
  ...
  - sha1: fails to start s_client
    - Created a certificate using sha1 instead of sha256

  - des: currently works
    - Created a certificate using a key generated by des3

  - mismatchkey: fails to start s_client
    - Tried to provide a different key to the cert

  - broken: fails to start s_client
    - Tried to edit a bit of the cert 

  - brokenKey: fails to start s_client
    - Tried to edit a bit of the key 
...
Certificate for others to use when encrypting files to you: 
  - For this requirement, I created a client certificate which can be found in
    intermediate/certs/www.example_client_encryption.com.cert.pem
    - What changed was that dataEncryption is the only specified keyUsage
 
Certificate suitable for signing files, including encrypted files
  - For this requirement, I created a client certificate which can be found
    in intermediate/certs/www.example_client_signing.com.cert.pem
    - What changed was that digitalSignature is the only specified keyUsage
