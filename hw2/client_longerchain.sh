...
openssl s_client -connect localhost:$PORT -cert $HOMEINTER/certs/www.example_client_longerchain.com.cert.pem -cert_chain $HOMEINTER/certs/ca-chain2.cert.pem -key $HOMEINTER/private/www.example_client_longerchain.com.key.pem -pass pass:pass -verify $DEPTH -verify_return_error -CApath $HOMEROOT/certs -CAfile $HOMEROOT/certs/ca.cert.pem -prexit -quiet
